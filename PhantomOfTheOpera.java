...
public class PhantomOfTheOpera extends Unit
{
	/*every single card should have a constructor that takes in a x,y location and a width/height, and a player it belongs to.*/
	public PhantomOfTheOpera(float x,float y,int width,int height,Player player,SelectCardGUI selectCardGUI) {
	...
	public PhantomOfTheOpera(PhantomOfTheOpera fk) {
		super(fk);
	...
	private void init() {
		...
		setName("Phantom of the Opera");//the name is just what the card is called.
		setSubType(SubType.THIEF);//the subtype is what archetype they are so like animal for dog or crown for king.
		setNameId(NameID.PHANTOM_OF_THE_OPERA);//YOU MUST HAVE THIS! you set the nameId to NameID.WHATEVER. because NameID is an enum.
	...
	public Card makeCopy() {
		return new PhantomOfTheOpera(this);
	}
