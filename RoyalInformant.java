...
public class RoyalInformant extends Unit implements EnterTrigger{
	/*every single card should have a constructor that takes in a x,y location and a width/height, and a player it belongs to.*/
	public RoyalInformant(float x,float y,int width,int height,Player player, SelectCardGUI selectCardGUI) {
	...
	public RoyalInformant(RoyalInformant fk, SelectCardGUI selectCardGUI) {
		super(fk);
	...
	private void init(SelectCardGUI selectCardGUI) {
		...
		setName("Royal Informant");//the name is just what the card is called.
		setSubType(SubType.THIEF);//the subtype is what archetype they are so like animal for dog or crown for king.
		setNameId(NameID.ROYAL_INFORMANT);//YOU MUST HAVE THIS! you set the nameId to NameID.WHATEVER. because NameID is an enum.
	...
	/*returns a new RoyalInformant using the copy constructor so you pass this in as the parameter because your are just making a copy.*/
	public Card makeCopy() {
		return new RoyalInformant(this, selectCardGUI);
	}
	...
	public void enterTrigger(BoardGUI boardGUI,PlayerHandler playerHandler,PlayerHandler opponentHandler) {
		//Card[] cards = new Card[opponent.getHand().getSize()];
